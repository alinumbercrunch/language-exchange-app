...
export async function apiRequest<T>(endpoint: string, options: ApiRequestOptions = {}): Promise<T> {
	...
	try {
		...
		throw new AppError(error instanceof Error ? error.message : "Network error occurred", 500);
	}
}
 ...
 */
export const api = {
	get: <T>(endpoint: string, headers?: Record<string, string>) => apiRequest<T>(endpoint, { method: "GET", headers }),
